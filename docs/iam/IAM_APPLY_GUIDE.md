...
| `dbaops-deployer-policy-2-data.json` | data + auth + observability (S3/IAM/Cognito/Secrets/Logs/Bedrock/STS/**DynamoDB state lock**/**RDS·Kafka describe for customer lookup**) | 3,645 자 — managed OK |
...
aws rds describe-db-instances --max-records 20 --no-cli-pager --query 'DBInstances[*].DBInstanceIdentifier'
aws kafka list-clusters-v2 --no-cli-pager --query 'ClusterInfoList[*].ClusterName'
